...
type (
	...
	OutboxStore interface {
		...
		Save(context.Context, asyncmessages.Message) error
		...
		FindUnpublished(context.Context, int) ([]asyncmessages.Message, error)
...
func OutboxPublisher(store OutboxStore) asyncmessages.MessagePublisherMiddleware {
